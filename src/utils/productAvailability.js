...
function getCurrentIngredientLevels(db) {
  try {
    ...
    if (Object.keys(ingredientLevels).length === 0) {
      console.log('⚠️ No device status found for any machine, assuming all ingredients available');
      return {};
    }

    console.log('📊 Total ingredient levels loaded:', Object.keys(ingredientLevels).length);
